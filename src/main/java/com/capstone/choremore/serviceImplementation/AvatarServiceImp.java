...
public class AvatarServiceImp implements AvatarService {
    ...
    public String getAvatarImg(Avatar myAv) {

        if(myAv.getImage() == null) {
    ...
    public Avatar getAvatarByMessage(Message message) {

        User user = message.getChild();

        return avatarDao.findAvatarByChildId(user.getId());

    }

    public Avatar getAvatarByChore (Chore chore) {

        User user = chore.getChild();
    ...
    public void skillBuilderView(Model model) {

        Avatar myAv = getCurrentAvatar();

        model.addAttribute("avatar", myAv);

        String base64Encoded = getAvatarImg(myAv);

        myAv.setImageString(base64Encoded);

        model.addAttribute("contentImage", myAv.getImageString());
        model.addAttribute("avatar", showAvatarByChildId());
